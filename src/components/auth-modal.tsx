...
export function AuthModal({ isOpen, onClose, conversionCount = 1 }: AuthModalProps) {
  const { signInWithGoogle, signInWithGithub, devLogin } = useAuth()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-2xl">
        ...
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-purple-600 shadow-lg shadow-primary/20">
            <Sparkles className="h-8 w-8 text-white" />
          </div>
          <h2 className="mb-2 text-2xl font-bold">Sign in to continue</h2>
          <p className="text-muted-foreground">
            {conversionCount >= 1 
              ? "You've used your free conversion! Sign in to unlock unlimited conversions and save your work."
        ...
        <div className="space-y-3">
          <Button
            ...
            className="h-12 w-full gap-3 rounded-xl bg-white text-gray-900 hover:bg-gray-100"
          >
            {isLoading === "google" ? (
        ...
        <div className="mt-8 rounded-xl bg-muted/50 p-4">
          <h4 className="mb-3 text-sm font-semibold">What you get:</h4>
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
              Save & access your assets anywhere
            </li>
            <li className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
